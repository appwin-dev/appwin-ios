...
actor AdIdentityReporter {
  ...
  private func sync() async {
    ...
    // Same identifier is only skipped while the last report is fresh: the
    // request itself carries matching signals the server captures (IP,
    // user agent), and those go stale even when the IDFA does not.
    let sentAt = prefs.string(forKey: keyPrefix + Self.sentAtKey).flatMap(TimeInterval.init) ?? 0
    let fresh = Date().timeIntervalSince1970 - sentAt < Self.resendTtlSeconds
    guard prefs.string(forKey: keyPrefix + Self.sentKey) != marker || !fresh else { return }
    ...
    if await send({ try await $0.requestVoid(path: Self.endpoint, httpMethod: .put, body: body) }) {
      ...
      prefs.set(String(Date().timeIntervalSince1970), forKey: keyPrefix + Self.sentAtKey)
    }
  }
  ...
  private static let sentAtKey = "adid.sentAt"
  /// Refresh cadence of an unchanged report (matching-signal freshness).
  private static let resendTtlSeconds: TimeInterval = 24 * 3600
