...
public enum AppwinInAppBanner {
  ...
  /// Shows `banner`, or drops it. Returns whether a card was attached.
  ///
  ...
  @discardableResult
  public static func present(_ banner: AppwinBanner) -> Bool {
    guard !isSuppressed else { return false }
    guard banner.id != lastShownID || showing == nil else { return false }
    guard let window = foregroundWindow() else { return false }
  ...
  ///
  /// Prefer the key window of a foreground scene, then any visible window in
  /// that scene. Fall back to `UIApplication.shared.windows`: React Native
  /// (and some Flutter hosts) still create a classic `UIWindow(frame:)` that
  /// is key but not listed under a scene's `windows`, which made the banner
  /// silently no-op while out-of-app APNs kept working.
  private static func foregroundWindow() -> UIWindow? {
    ...
    if let key = UIApplication.shared.windows.first(where: \.isKeyWindow) { return key }
    return UIApplication.shared.windows.first { !$0.isHidden && $0.alpha > 0 }
  }
