...
public enum AppwinCore {
  ...
  // Read-only here; mutate through `configure` / `identify` / `logout`.
  ...
  /// External user id passed to `identify(externalId:attributes:)`, persisted
  /// across launches. `nil` means anonymous (lead) mode.
  ...
  /// The hub without creating it: an identity change must not open a socket.
  static var realtimeHubIfCreated: RealtimeHub? { _realtimeHub }
  ...
  /// Replayed by `logout()`: the server drops the device's push tokens on
  /// revoke, and the OS will not hand the app a new token by itself.
  static var lastPushRegistration: PushTokenRegistration?
  ...
  /// Set by AppwinNotifications when that product is linked. Community (and
  /// hosts) call [preparePushCapture] so cold-start notification taps are
  /// queued before the feed mounts - without importing Notifications.
  public static var pushCapturePreparer: (() -> Void)?

  /// Arms the push notification center delegate when Notifications is linked.
  /// Safe no-op otherwise. Call from Community.initialize / host launch.
  public static func preparePushCapture() {
    pushCapturePreparer?()
  }
  ...
  /// bearer, carrying the externalId persisted by a previous `identify`.
  public static func configure(
    ...
    let storedExternalId = KeychainStore.get(externalIdKey)
    IdentityStore.mutate {
      $0.deviceId = deviceId
      $0.externalId = storedExternalId
    }
  ...
  public static func registerPushToken(
    ...
    lastPushRegistration = PushTokenRegistration(token: token, platform: platform, pushOptIn: pushOptIn)
  }
  ...
  static let deviceIdKey = "appwin.core.deviceId"
  static let externalIdKey = "appwin.core.externalId"
