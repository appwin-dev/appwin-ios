...
public enum AppwinCore {
  ...
  ///
  /// Synchronous on purpose: it prepares device id, client and cached token
  /// immediately, then opens the server session in the background. Blocking
  /// startup on a round trip would be paid by every user, offline ones
  /// included. To force or await it, use `bootstrapSession(externalId:)`.
  public static func configure(
    ...
    Task { @MainActor in
      try? await Self.bootstrapSession()
    }
  }
  ...
  public static func reportUnavailable(_ product: AppwinProduct, _ result: AppwinInitResult) {
    ...
    case .unknown:
      detail = "no verdict yet: offline on a first launch. Retry initialize()."
