...
public final class RealtimeHub: @unchecked Sendable {
  ...
  /// Reconnects with a freshly minted token, only if a connection was up.
  ///
  /// The topics live in the token claims (`sdk:customer:{id}`), so after the
  /// session switched customer the open socket keeps listening to the previous
  /// one until it reconnects.
  package func restartIfRunning() {
    lock.lock()
