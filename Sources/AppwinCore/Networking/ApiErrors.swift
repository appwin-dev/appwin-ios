...
public enum AppwinApiError: Error {
  ...
  /// `configure` has not run, so there is no client to send with.
  case notConfigured
  /// A caller passed something the SDK refuses before any request.
...
extension AppwinApiError {
  ...
  public var userMessage: String {
    ...
    // Server side: auth refused (wrong base URL, revoked token, unknown app id).
    case .http(let status) where status == 401 || status == 403:
      return "Connexion au support refusée. Vérifiez la configuration et réessayez."
    ...
    case .invalidUrl, .encodingFailed, .decodingFailed, .notConfigured, .invalidArgument:
