...
struct CommunityMediaDTO: Decodable {
    ...
    func toDomain() -> CommunityMedia? {
        ...
        return CommunityMedia(
            ...
            height: height,
            alt: alt,
            type: CommunityMedia.resolveType(apiType: type, url: url)
        )
    }
}
...
struct CommunityPostDTO: Decodable {
    ...
    let reactionCounts: [CommunityReactionCountDTO]?
    let canEdit: Bool
    let canDelete: Bool
    ...
    func toDomain() -> CommunityPost {
        CommunityPost(
            ...
            topReactions: (topReactions ?? []).compactMap(CommunityReactionKind.init(rawValue:)),
            reactionCounts: (reactionCounts ?? []).compactMap { $0.toDomain() },
            canEdit: canEdit,
            canDelete: canDelete,
...
struct CommunityReactionCountDTO: Decodable {
    let kind: String
    let count: Int

    func toDomain() -> CommunityReactionCount? {
        guard let kind = CommunityReactionKind(rawValue: kind), count > 0 else { return nil }
        return CommunityReactionCount(kind: kind, count: count)
    }
}
...
struct UpdatePostBody: Encodable {
    let body: String?
    let groupId: String?
    let media: [CreatePostBody.MediaInput]?

    enum CodingKeys: String, CodingKey {
        case body, groupId, media
    }
...
struct CreateCommentBody: Encodable {
    ...
    init(body: String, parentCommentId: String?, media: [CommunityMedia] = []) {
        self.body = body
