...
public enum AppwinCommunity {
    ...
    /// Whether `initialize()` has returned `.ready`. Rendering is gated on it.
    ...
    public static func communityView() -> AnyView {
        // A neutral view rather than a crash or a blank feed: this is the
        // embedded path, and it may already be mounted in a tab when a plan
        // lapses mid-session.
        guard isReady else {
            AppwinCore.reportUnavailable(.community, .unavailable(.disabled))
            return AnyView(EmptyView())
        }
        return AnyView(AppwinCommunityRootView())
    }
    ...
    public static func communityViewController() -> UIViewController {
        ...
        return UIHostingController(rootView: AppwinCommunityRootView())
    }
