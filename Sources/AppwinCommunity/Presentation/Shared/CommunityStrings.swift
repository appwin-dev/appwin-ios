...
///
/// The package ships `en.lproj` / `fr.lproj` `AppwinCommunity.strings`. On RN /
/// Flutter hosts, `Bundle.main` often stays English even when the phone is
/// French - we pick the `*.lproj` from `Locale.preferredLanguages` (same as
/// Support). A studio can still override any key by shipping the same table in
/// the host app (Bundle.main wins when looked up first - see [t(_:_:)]).
enum CommunityStrings {
    private static func t(_ key: String, _ fallback: String) -> String {
        ...
        let bundle = AppwinDisplayLocale.bundle(languageCode: nil, resources: resourceBundle)
    ...
    static var editPost: String { t("community.edit_post", "Edit post") }
    static var edit: String { t("community.edit", "Edit") }
    ...
    static var replies: String { t("community.replies", "Replies") }
    static var replyToComment: String {
        t("community.reply_to_comment", "Reply to comment")
    }
    static func showReplies(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.show_replies_one", "→ See %d reply"), count)
        }
        return String(format: t("community.show_replies", "→ See %d replies"), count)
    }
    ...
    static var composerAnonymousHint: String {
        t(
            "community.composer_anonymous_hint",
            "You're posting anonymously. Set up your profile so members can recognize you."
        )
    }
    static var composerLeaveAnonymous: String {
        t("community.composer_leave_anonymous", "Set up my profile")
    }
    static var memberSince: String { t("community.member_since", "Member since %@") }
    /// Singular for 0 and 1 (French UI expects "0 commentaire"; English
    /// "0 comment" is acceptable for these feed counters).
    private static func usesSingularForm(_ count: Int) -> Bool {
        count >= 0 && count <= 1
    }

    static func postCount(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.post_count_one", "%d post"), count)
        }
        return String(format: t("community.post_count", "%d posts"), count)
    }
    static func commentCount(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.comment_count_one", "%d comment"), count)
        }
        return String(format: t("community.comment_count", "%d comments"), count)
    }
    static func reactionCount(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.reaction_count_one", "%d like"), count)
        }
        return String(format: t("community.reaction_count", "%d likes"), count)
    }
    static func commentCountShort(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.comment_count_short_one", "%d comment"), count)
        }
        return String(format: t("community.comment_count_short", "%d comments"), count)
    }
    static func viewCountShort(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.view_count_short_one", "%d view"), count)
        }
        return String(format: t("community.view_count_short", "%d views"), count)
    }
    static var beFirstToReact: String {
        t("community.be_first_to_react", "Be the first to react")
    }
    static func reactionsTitle(_ total: Int) -> String {
        if usesSingularForm(total) {
            return String(format: t("community.reactions_title_one", "%d reaction"), total)
        }
        return String(format: t("community.reactions_title", "%d reactions"), total)
    }
    ...
    static func postDetailTitle(_ nickname: String) -> String {
        String(format: t("community.post_detail_title", "Post by %@"), nickname)
    }
    ...
    static var videoTooLarge: String {
        t("community.video_too_large", "This video exceeds the 75 MB limit.")
    }
    ...
    static func pollVotes(_ count: Int) -> String {
        if usesSingularForm(count) {
            return String(format: t("community.poll_votes_one", "%d vote"), count)
        }
        return String(format: t("community.poll_votes", "%d votes"), count)
    }
    ...
    static var relativeNow: String { t("community.relative_now", "now") }
...
extension Bundle {
    static let appwinCommunityResources: Bundle = {
        ...
        // CocoaPods: AppwinCommunity.bundle. SPM process("Resources") in a
        // host that still hits this path: Appwin_AppwinCommunity.bundle.
        let names = ["AppwinCommunity.bundle", "Appwin_AppwinCommunity.bundle"]
