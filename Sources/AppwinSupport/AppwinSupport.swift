...
public enum AppwinSupport {
  ...
  ///
  /// Call it after `AppwinCore.configure(projectAppId:)` and **before** showing
  /// any Support entry point, then gate your own UI on the result: the SDK
  /// cannot hide your help button or your tab, it does not own your navigation.
  ///
  /// ```swift
  ...
  public static func presentMessenger() {
    // Refused rather than half-presented: a modal that opens on an empty screen
    // is harder to diagnose than one that never opens and says why.
    guard isReady else {
    ...
    guard let root = makeMessengerRoot(onClose: { dismissPresented?() }) else { return }
