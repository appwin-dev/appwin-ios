...
public enum AppwinSupport {
  ...
  /// Session of the open messenger, so `updateUser` refreshes the avatar live.
  ...
  public static func initialize() async -> AppwinInitResult {
    ...
    if !result.isReady {
      ...
      AppwinPushRouting.handleDeeplink = nil
    } else {
      ...
      AppwinPushRouting.handleDeeplink = { url in
        guard let conversationId = AppwinPushRouting.supportConversationId(from: url) else {
          return false
        }
  ...
  public static func loginUnidentifiedUser() async throws -> Customer {
    ...
    // Mint (or refresh) the bearer before identify. Without this, a launch
    // that got `.ready` from the availability cache while `/auth/init` failed
    // still sends a dead or missing token, and the guard answers 401.
    _ = try await AppwinCore.bootstrapSession()
    // No display name: the device is the identity. Inventing
    // "Visitor anonyme \(uuid)" leaked into the studio inbox.
  ...
  /// Clears the in-memory customer so the next messenger open creates a lead.
  @MainActor
  public static func clearLocalCustomer() {
    customer = nil
  ...
  /// Re-fetches the customer (language may have been updated by inbound detection).
  @discardableResult
  public static func refreshCustomer() async throws -> Customer {
    try await updateUser(attributes: AppwinSupportUserAttributes())
  }
