...
struct MessengerView: View {
  ...
  private func submitMessage() {
    ...
    // Capture and clear the draft without remounting the TextField - remounting
    // drops focus and the keyboard briefly dismisses then reopens.
    ...
    Task {
      ...
      guard let id = activeConversation?.id else {
        // No conversation yet: show the bubble now, then create the thread.
        let localId = messageStore.insertOptimisticCustomerMessage(body: body)
        if let conv = await conversationStore.start(firstMessage: body, attachments: refs) {
          createdConversation = conv
          // `.task` will reload; keep the optimistic bubble until that lands.
          _ = localId
          // Load / layout can steal focus once the thread id appears.
          isComposerFocused = true
        } else {
          messageStore.removeLocalMessage(id: localId)
        }
        return
      }
  ...
  var body: some View {
    ...
    // `nil` means a new conversation, so there is nothing to load.
      .task(id: activeConversation?.id) {
        // The in-app banner reads this: a reply to the thread on screen is
        // already visible, so it is not announced over it.
        OpenThread.conversationId = activeConversation?.id
        guard let id = activeConversation?.id else {
        ...
        // Left during load: do not mark read (would clear the pastille).
        guard OpenThread.conversationId == id else { return }
    ...
    // draft would leak onto the next conversation.
      .onDisappear {
        ...
        composerStore.clear()
        cancelEdit()
        OpenThread.conversationId = nil
        // Drop the loaded thread id: otherwise a realtime resync while the
        // user is back on home would markRead and clear the unread badge
        // without them opening the conversation.
        messageStore.leaveConversation()
      }
...
private struct PreviewMessageRepository: MessageRepository {
    ...
    CursorPaginated(
      items: [
        Message(id: "1", authorType: .organizationMember, authorName: "Studio",
                body: "Bonjour 👋 comment puis-je vous aider ?", translatedBody: nil, readAt: nil, createdAt: .now, attachments: [], reactions: []),
        Message(id: "2", authorType: .customer, authorName: nil,
                body: "Salut, j'ai une question sur ma commande", translatedBody: nil, readAt: nil, createdAt: .now, attachments: [], reactions: []),
        Message(id: "3", authorType: .organizationMember, authorName: "Studio",
                body: "Bien sûr, donnez-moi votre numéro de commande", translatedBody: nil, readAt: nil, createdAt: .now, attachments: [], reactions: []),
      ],
      nextCursor: nil
    )
  }
  func send(conversationId: String, body: String, attachments: [AttachmentInput]) async throws -> Message {
    Message(id: "x", authorType: .customer, authorName: nil, body: body, translatedBody: nil, readAt: nil, createdAt: .now, attachments: [], reactions: [])
  }
  func update(conversationId: String, messageId: String, body: String) async throws -> Message {
    Message(id: messageId, authorType: .customer, authorName: nil, body: body, translatedBody: nil, readAt: nil, createdAt: .now, attachments: [], reactions: [])
  }
  ...
  func toggleReaction(conversationId: String, messageId: String, emoji: String) async throws -> Message {
    Message(id: messageId, authorType: .customer, authorName: nil, body: "", translatedBody: nil, readAt: nil, createdAt: .now, attachments: [], reactions: [
...
#Preview {
  ...
  let conversation = Conversation(
    ...
    lastMessageAt: nil, lastMessageAuthorType: nil, lastReadAt: nil, createdAt: .now
  )
