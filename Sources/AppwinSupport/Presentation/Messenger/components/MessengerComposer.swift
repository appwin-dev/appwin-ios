...
struct MessengerComposer: View {
    ...
    /// Forces the TextField to remount after a reset; SwiftUI sometimes keeps
    /// the old text while focused.
    var fieldResetId: UUID = UUID()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            TextField("Écris ton message...", text: $text, axis: .vertical)
                .id(fieldResetId)
                .font(.system(size: 14))
            ...
            HStack(spacing: 4) {
                if !isEditing {
                    toolbarButton(.gallery, label: "Ajouter une image") {
                        showImagePicker = true
                    }
                    toolbarButton(.videoLibrary, label: "Ajouter une vidéo") {
                        showVideoPicker = true
                    }
                    toolbarButton(.paperclip, label: "Joindre un fichier") {
                ...
                Button(action: onSend) {
                    HStack(spacing: 4) {
                        Text(isEditing ? "Enregistrer" : "Envoyer")
