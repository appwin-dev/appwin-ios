...
enum MessageDayGrouping {
    ...
    static func items(
        ...
        while index < chronological.count {
            ...
            result.append(
                .day(
                    id: dayKey(dayStart),
                    label: formatDayLabel(dayStart, language: language, calendar: calendar)
                )
            )
    ...
    static func formatDayLabel(
        _ date: Date,
        language: String? = nil,
        calendar: Calendar = .current
    ) -> String {
        if calendar.isDateInToday(date) { return SupportStrings.today(language: language) }
        if calendar.isDateInYesterday(date) { return SupportStrings.yesterday(language: language) }

        let formatter = DateFormatter()
        formatter.locale = SupportDisplayLocale.locale(languageCode: language)
