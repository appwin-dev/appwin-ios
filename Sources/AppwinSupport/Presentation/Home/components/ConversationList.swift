...
struct ConversationList: View {
                ...
                if let onStartConversation {
                    Button(action: onStartConversation) {
                        HStack(spacing: 10) {
                            ...
                            Text(SupportStrings.newConversationPreview)
    ...
    private var emptyState: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 6) {
                Text(SupportStrings.noConversation)
                    .font(.system(size: 16, weight: .semibold))
                    .foregroundColor(AppwinTokens.textHigh)
                Text(SupportStrings.emptyConversationsHint)
            ...
            if let onStartConversation {
                Button(action: onStartConversation) {
                    Text(SupportStrings.writeMessage)
    ...
    private func errorState(_ error: Error) -> some View {
        VStack(spacing: 12) {
            Text(SupportStrings.loadErrorTitle)
