...
struct HomeView: View {
  ...
  private func bootstrapCustomer() async {
    do {
      _ = try await AppwinSupport.ensureCustomer()
      session.setCustomer(try await AppwinSupport.refreshCustomer())
      bootstrapError = nil
    } catch {
  ...
  private func failure(_ error: Error) -> some View {
    VStack(spacing: 12) {
      Text(SupportStrings.messengerOpenFailed)
    ...
    VStack(spacing: 0) {
      ScrollView {
        VStack(alignment: .leading, spacing: 24) {
          ...
          if let recent = recentOpenConversation {
            NavigationLink(value: AppwinRoute.messenger(recent)) {
              RecentMessageCard(conversation: recent)
            }
            .buttonStyle(.plain)
          }
          ...
          VStack(spacing: 12) {
            ...
            NavigationLink(value: AppwinRoute.conversations) {
              ConversationsCard(hasUnread: inboxHasUnread)
            }
            .buttonStyle(.plain)
          }
  ...
  private var inboxHasUnread: Bool {
    conversationStore.conversations.contains { $0.hasUnread }
  }

  /// Latest open thread for the Intercom-style home teaser.
  private var recentOpenConversation: Conversation? {
  ...
  private var greeting: some View {
    let firstName = session.customer?.name?
      ...
      .split(separator: " ")
      .first
      .map(String.init) ?? SupportStrings.greetingYou

    return VStack(alignment: .leading, spacing: 0) {
      Text(String(format: SupportStrings.greetingNamed, firstName))
      Text(SupportStrings.greetingSubtitle)
    }
