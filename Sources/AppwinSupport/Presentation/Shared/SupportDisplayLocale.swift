...
/// typically English). An explicit `languageCode` (from `updateUser`) still
/// wins when the studio or host sets it on purpose.
enum SupportDisplayLocale {
  ...
  /// Language tag for Support UI strings. Prefer an explicit customer language;
  /// otherwise the device preferred language (SDK `fr.lproj` / `en.lproj`).
