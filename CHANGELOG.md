...
| 0.7 | 0.8 |
|---|---|
| `AppwinCore.identify(externalId:)` (local only) | `try await AppwinCore.identify(externalId:attributes:)` |
| `AppwinSupport.loginIdentifiedUser(externalId:)` | `try await AppwinCore.identify(externalId:attributes:)` |
| `AppwinSupport.updateUser(attributes:)`, `refreshCustomer()` | `try await AppwinCore.updateUser(_:)` |
| `AppwinSupport.loginUnidentifiedUser()`, `ensureCustomer()`, `clearLocalCustomer()` | nothing: Core opens the anonymous session itself |
| `AppwinCommunity.login(externalId:)` | `try await AppwinCore.identify(externalId:attributes:)` |
| `AppwinCore.signOut()`, `AppwinCommunity.logout()`, `clearIdentity()` | `await AppwinCore.logout()` |
| `AppwinCore.bootstrapSession(externalId:)` | nothing: internal now |
| `AppwinNotifications.registerPushToken(...)` | `AppwinCore.registerPushToken(...)` |
| `AppwinNotifications.handleRemoteNotification` | `AppwinPush` |
...
**A tap on an Appwin push opens the right screen, cold start included.** A
Support reply now opens its conversation even when the tap launches the app:
the tap used to reach the SDK before `AppwinSupport.initialize()` had
registered anything, and was handed to iOS as an `appwin://` URL no app
declares. Taps are now kept until the owning product is ready, then replayed.

**New: `AppwinPush`, in Core.** One router for every product's pushes
(Support, Notifications, Community later), and the only API an app that owns
its push stack (Firebase Messaging, FlutterFire) needs: forward from your own
callbacks to `AppwinPush.handleTap(_:)`, `handleForeground(_:)` and
`handleMessage(_:)`, and test with `isAppwinPush(_:)`. Same names on Android.
`handleForeground(_:title:body:)` takes the notification text for payloads that
reach you without `aps` (Firebase Messaging hands `data` and text apart).

**The notification delegate no longer replaces yours.** `start()` still takes
the `UNUserNotificationCenter` delegate slot, but hands every notification that
is not Appwin's to the delegate it displaced. To keep the slot entirely,
`start(installsNotificationDelegate: false)` and forward to `AppwinPush`.

**Deprecated:** `AppwinNotifications.handleRemoteNotification(_:)` (use
`AppwinPush.handleMessage(_:)`), and `AppwinPushRouting` (its `handleDeeplink`
hook is no longer consulted). In-app message deeplinks to `appwin://` routes
are now routed inside the SDK too.

## 0.6.3

**The TikTok adapter no longer double-counts purchases.** Its StoreKit
auto-tracking is disabled, same governance as its SKAN: the app's
`track("purchase", {value, currency})` - relayed with our shared event id -
is the single purchase signal. See the wiki for where to place that call per
payment stack (RevenueCat, StoreKit 2).
